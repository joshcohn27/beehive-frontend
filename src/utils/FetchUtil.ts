...
const loadMockData = async (): Promise<MockApiData> => {
  ...
  mockDataPromise = (async () => {
    ...
    const data = (await response.json()) as MockApiData;
...
const saveMockData = (data: MockApiData) => {
  window.localStorage.setItem(MOCK_STORAGE_KEY, JSON.stringify(data));
  mockDataPromise = Promise.resolve(data);
};
...
const buildSensorResponse = (hive: MockHive, sensorName: string, filteredData: MockHiveDataPoint[]) => {
  ...
  if (sensorName === "/CarbonDioxide") {
    return {
      Hive_ID: Number(hive.Hive_ID),
      Carbon_Dioxide_Data: filteredData.map((entry) => ({
        TimeStamp: entry.TimeStamp,
...
const getObservationDeleteId = (body: Record<string, any>): string =>
  String(
    body.Observation?.Observation_ID ??
    body.Observations?.Observation_ID ??
    body.Observation_ID ??
    ""
  );

const getInspectionDeleteId = (body: Record<string, any>): string =>
  String(
    body.Inspection?.Inspection_ID ??
    body.Inspections?.Inspection_ID ??
    body.Inspection_ID ??
    ""
  );
...
const handleMockRequest = async <T>(pathOrUrl: string, options: FetchJsonOptions): Promise<T> => {
  ...
  if (url === "/Observations") {
    ...
    if (method === "DELETE") {
      const observationId = getObservationDeleteId(body);

      if (!observationId) {
        throw new ApiError("Observation ID is required.", {
          status: 400,
          url,
          body,
        });
      ...
      const exists = hive.Observations.some(
        (observation) => String(observation.Observation_ID) === observationId
      );

      if (!exists) {
        throw new ApiError("Observation not found.", {
          status: 404,
          url,
          body,
        });
  ...
  if (url === "/Inspections") {
    ...
    if (method === "DELETE") {
      const inspectionId = getInspectionDeleteId(body);

      if (!inspectionId) {
        throw new ApiError("Inspection ID is required.", {
          status: 400,
          url,
          body,
        });
      ...
      const exists = hive.Inspections.some(
        (inspection) => String(inspection.Inspection_ID) === inspectionId
      );

      if (!exists) {
        throw new ApiError("Inspection not found.", {
          status: 404,
          url,
          body,
        });
