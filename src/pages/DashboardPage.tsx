...
const DashboardPage = () => {
	...
	function fetchHiveData() {
		...
		const thirtyDaysAgo = dateToSqlTimestamp(new Date(Date.now() - 30 * 24 * 60 * 60 * 1000));
		...
		fetchJson<AllHiveDataResponse>(`/HiveData`, {
			...
			.then((response) => {
				const pruned = pruneHiveData(response);
				setPrunedData(pruned);
				setCache(String(hiveNumber), pruned);
				setError(null);
			})
			.catch((error) => {
				if (error.status === 400) {
					setError("400: Warning; No data available for this hive.");
				} else {
	...
	function pruneHiveData(data: AllHiveDataResponse | null) {
		...
		const latestEntry = sortedRows[sortedRows.length - 1];
		...
		fetchJson<any>(`/Observations`, {
			...
			.then((response) => {
				...
				const sorted = [...raw].sort((a, b) => {
					const timeDiff =
						parseTimestamp(b.TimeStamp ?? b.Timestamp ?? b.timestamp ?? "") -
						parseTimestamp(a.TimeStamp ?? a.Timestamp ?? a.timestamp ?? "");
				...
				setLatestObservation({
					...
					date: normalizeDateString(latest.TimeStamp ?? latest.Timestamp ?? latest.timestamp ?? ""),
				});
			})
			.catch((error) => {
	...
	async function handleSubmitObservation(draft: ObservationDraft) {
		...
		try {
			...
			const createdObservation: Observation = { id: `pending-${Date.now()}`, ...draft };
			setLastAddedObservation(createdObservation);

			setLatestObservation({
				headline: draft.tags && draft.tags.length > 0 ? draft.tags.join(", ") : "No tags",
				body: draft.notes,
				date: normalizeDateString(draft.date),
			});
